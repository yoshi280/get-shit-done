 ...
 *     --stopped-at "..."
 *     [--resume-file path]
 *   state update-cost                  Write phase cost to STATE.md Cost Tracker
 *     --phase N --dimensions "a,b,c"
...
function cmdStateUpdateCost(cwd, options, raw) {
  const statePath = path.join(cwd, '.planning', 'STATE.md');
  if (!fs.existsSync(statePath)) { output({ error: 'STATE.md not found' }, raw); return; }

  const { phase, dimensions, input_tokens, output_tokens, cost } = options;

  if (!phase || !dimensions || input_tokens === null || input_tokens === undefined ||
      output_tokens === null || output_tokens === undefined || cost === null || cost === undefined) {
    output({ error: 'phase, dimensions, input-tokens, output-tokens, and cost are required' }, raw);
    return;
  }

  const inputNum = parseInt(input_tokens, 10);
  const outputNum = parseInt(output_tokens, 10);
  const costNum = parseFloat(cost);

  if (isNaN(inputNum) || isNaN(outputNum) || isNaN(costNum)) {
    output({ error: 'input-tokens and output-tokens must be integers, cost must be a number' }, raw);
    return;
  }
  ...
  const newRow = `| ${phase} | ${dimensions} | ${fmt(inputNum)} | ${fmt(outputNum)} | ${fmtCost(costNum)} |`;

  let content = fs.readFileSync(statePath, 'utf-8');

  // Check if Cost Tracker section exists; if not, create it
  const costTrackerPattern = /## Cost Tracker\n([\s\S]*?)(?=\n## |\n$|$)/;
  let hasCostTracker = costTrackerPattern.test(content);

  if (!hasCostTracker) {
    // Insert Cost Tracker section before ## Accumulated Context (or at end of Performance Metrics)
    const insertBefore = /## Accumulated Context/;
  ...
  // Re-read section after potential insertion
  const sectionMatch = content.match(/## Cost Tracker\n([\s\S]*?)(?=\n## (?!Cost Tracker)|\s*$)/);
  if (!sectionMatch) {
    output({ error: 'Failed to locate Cost Tracker section after creation attempt' }, raw);
    return;
  }

  let sectionBody = sectionMatch[1];

  // Extract pruned total from comment if present
  let prunedTotal = 0;
  const prunedMatch = sectionBody.match(/<!--\s*pruned_total:\s*([\d.]+)\s*-->/);
  if (prunedMatch) {
    prunedTotal = parseFloat(prunedMatch[1]) || 0;
  }

  // Parse existing table rows (skip header, separator, placeholder, and comment lines)
  const lines = sectionBody.split('\n');
  const dataRows = lines.filter(l => {
    const trimmed = l.trim();
    if (!trimmed.startsWith('|')) return false;
    if (/^\|\s*[-|]+\s*$/.test(trimmed)) return false; // separator line
    if (/^\|\s*Phase\s*\|/i.test(trimmed)) return false; // header line
    if (/^\|\s*-\s*\|/.test(trimmed)) return false; // placeholder row
    return true;
  });
  ...
  // Cap at 10 rows — prune oldest (from front) if exceeded
  let newPrunedTotal = prunedTotal;
  while (dataRows.length > 10) {
    const pruned = dataRows.shift();
    // Extract cost value from pruned row to add to pruned total
    const costMatch = pruned.match(/\|\s*\$?([\d.]+)\s*\|?\s*$/);
    if (costMatch) {
      newPrunedTotal += parseFloat(costMatch[1]) || 0;
    }
  }

  // Compute running total = pruned total + sum of visible rows
  let visibleTotal = 0;
  for (const row of dataRows) {
    const costMatch = row.match(/\|\s*\$?([\d.]+)\s*\|?\s*$/);
    if (costMatch) {
      visibleTotal += parseFloat(costMatch[1]) || 0;
    }
  }
  const totalCost = newPrunedTotal + visibleTotal;

  // Rebuild the section body
  const today = new Date().toISOString().split('T')[0];
  const tableHeader = '| Phase | Dimensions Run | Input Tokens | Output Tokens | Cost (est.) |\n|-------|---------------|-------------|--------------|-------------|';
  const prunedComment = newPrunedTotal > 0 ? `<!-- pruned_total: ${newPrunedTotal.toFixed(3)} -->\n` : '';
  const newSection = `## Cost Tracker\n\n**Total project cost:** $${totalCost.toFixed(3)} (estimated)\n**Last updated:** ${today}\n\n${prunedComment}${tableHeader}\n${dataRows.join('\n')}\n`;

  // Replace the Cost Tracker section in content
  content = content.replace(/## Cost Tracker\n[\s\S]*?(?=\n## (?!Cost Tracker)|\s*$)/, newSection);

  fs.writeFileSync(statePath, content, 'utf-8');
  output({ success: true, phase, cost: fmtCost(costNum), total_cost: `$${totalCost.toFixed(3)}` }, raw, 'true');
}
...
async function main() {
  ...
  switch (command) {
    case 'state': {
      ...
      } else if (subcommand === 'update-cost') {
        const phaseIdx = args.indexOf('--phase');
        const dimsIdx = args.indexOf('--dimensions');
        const inputIdx = args.indexOf('--input-tokens');
        const outputIdx = args.indexOf('--output-tokens');
        const costIdx = args.indexOf('--cost');
        cmdStateUpdateCost(cwd, {
          phase: phaseIdx !== -1 ? args[phaseIdx + 1] : null,
          dimensions: dimsIdx !== -1 ? args[dimsIdx + 1] : null,
          input_tokens: inputIdx !== -1 ? args[inputIdx + 1] : null,
          output_tokens: outputIdx !== -1 ? args[outputIdx + 1] : null,
          cost: costIdx !== -1 ? args[costIdx + 1] : null,
        }, raw);
      } else {
