...
```bash
INIT_RAW=$(node /Users/thelorax/.claude/get-shit-done/bin/gsd-tools.cjs init progress --include state,roadmap,project,config)
...
```bash
ROADMAP=$(node /Users/thelorax/.claude/get-shit-done/bin/gsd-tools.cjs roadmap analyze)
```
...
- Use `summary-extract` for efficient parsing:
  ```bash
  node /Users/thelorax/.claude/get-shit-done/bin/gsd-tools.cjs summary-extract <path> --fields one_liner
  ```
...
PROGRESS_BAR=$(node /Users/thelorax/.claude/get-shit-done/bin/gsd-tools.cjs progress bar --raw)
```
...
Parse `state_content` (already loaded from init) to extract the ## Cost Tracker section.
1. Find the `**Total project cost:**` line — extract the dollar amount
2. Parse the Cost Tracker table rows (skip header, separator, and placeholder `| - |` rows)
3. For each data row, extract: phase (col 1), cost (col 5), dimensions (col 2)
4. Build the Cost Summary table from these rows
5. If no data rows exist (section absent, or only placeholder rows), **omit the entire Cost Summary section** from the report
6. Display costs without judgment — no warning symbols, no language implying costs are high
