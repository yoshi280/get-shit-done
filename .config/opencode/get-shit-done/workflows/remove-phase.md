...
```bash
INIT=$(node /Users/thelorax/.config/opencode/get-shit-done/bin/gsd-tools.js init phase-op "${target}")
```
...
```bash
node /Users/thelorax/.config/opencode/get-shit-done/bin/gsd-tools.js commit "chore: remove phase {target} ({original-phase-name})" --files .planning/
```
