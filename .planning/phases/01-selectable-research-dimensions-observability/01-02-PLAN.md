...
must_haves:
  truths:
    ...
    - "plan-phase.md persists research cost to STATE.md via gsd-tools.cjs state update-cost"
  artifacts:
    - path: "~/.claude/get-shit-done/workflows/plan-phase.md"
      provides: "Dimension selection flow (Steps 5.1-5.5), live token display, and STATE.md cost persistence"
      contains: "Dimensions"
  ...
  key_links:
    ...
    - from: "~/.claude/get-shit-done/workflows/plan-phase.md"
      ...
      via: "Calls state update-cost after parsing token_report to persist cost to STATE.md"
...
<task type="auto">
  ...
  <action>
    ...
    **Add live token display AND cost persistence after researcher returns:**
    ...
    **Then persist the cost data to STATE.md** by calling gsd-tools.cjs state update-cost. After displaying the token report, parse the individual values from the token_report (dimensions, input_tokens, output_tokens, estimated_cost_usd) and call:
    ...
    This writes the per-phase cost row to STATE.md's Cost Tracker section, maintaining the cumulative project-wide cost tracker per locked decision. If the state update-cost command is not yet available (Plan 03 not executed), log a warning but do not fail -- the data is already displayed live and will be persisted on re-run or manually.
  ...
  <verify>
    ...
    5. A call to `gsd-tools.cjs state update-cost` exists after the token display, with parsed token_report values
    6. The state update-cost call is wrapped in graceful error handling (warn on failure, do not abort)
    7. Steps 1-4 and 6-14 are unchanged
    8. All existing skip conditions (--gaps, --skip-research, etc.) are preserved
  </verify>
  <done>plan-phase.md contains dimension selection sub-steps (5.1-5.5) that load the catalog, infer defaults, present a checklist via AskUserQuestion, handle custom dimensions, offer prompt editing, inject selected dimensions into the researcher prompt, display live token usage after research completes, and persist research cost to STATE.md via gsd-tools.cjs state update-cost.</done>
</task>
...
5. plan-phase.md calls gsd-tools.cjs state update-cost to persist cost to STATE.md after displaying token report
6. gsd-phase-researcher.md accepts parameterized dimensions with backward-compatible fallback
7. gsd-phase-researcher.md writes <token_report> footer to RESEARCH.md
8. No changes to plan-phase.md steps outside Step 5
9. No changes to researcher sections outside execution_flow and output_format
</verification>
...
- plan-phase.md persists research cost to STATE.md via state update-cost call
- Backward compatibility preserved for both plan-phase.md and researcher agent
- The system works end-to-end: user sees checklist, selects/edits dims, researcher runs selected dims, token report written, cost persisted to STATE.md
</success_criteria>
