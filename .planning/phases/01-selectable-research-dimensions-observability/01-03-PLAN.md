...
must_haves:
  truths:
    ...
    - "SUMMARY.md template includes a conditional Research Token Usage section with per-dimension cost breakdown"
  ...
  artifacts:
    ...
    - path: "~/.claude/get-shit-done/templates/summary.md"
      provides: "Research Token Usage section template for per-dimension cost detail"
      contains: "Research Token Usage"
  ...
  key_links:
    ...
    - from: "~/.claude/get-shit-done/templates/summary.md"
      to: "RESEARCH.md <token_report>"
      via: "Executor reads token_report from RESEARCH.md to populate Research Token Usage section"
      pattern: "token_report"
---

<objective>
Implement the observability backend: token price configuration, cost tracking command in gsd-tools.cjs, STATE.md Cost Tracker section, per-dimension token detail in SUMMARY.md template, and cost summary in progress reports.

Purpose: This is the data pipeline that makes research costs visible. The researcher (Plan 02) writes token_report footers; this plan builds everything that reads, stores, and displays that data. Together they fulfill the "informed, not anxious" cost visibility goal. Per the locked decision, token usage is displayed in three places: live output (Plan 02), per-dimension detail in SUMMARY.md (this plan), and per-phase aggregates in /gsd:progress (this plan).

Output: gsd-tools.cjs state update-cost command, STATE.md Cost Tracker section, config.json token_prices, summary.md Research Token Usage section, progress.md cost summary display.
</objective>
...
<task type="auto">
  <name>Task 1: Add token_prices to config.json, Cost Tracker to STATE.md template, and Research Token Usage to summary template</name>
  <files>
    .planning/config.json
    ~/.claude/get-shit-done/templates/state.md
    ~/.claude/get-shit-done/templates/summary.md
  ...
  <action>
    ...
    **3. Update ~/.claude/get-shit-done/templates/summary.md** to include a conditional `## Research Token Usage` section.

    Per the locked decision: "Token usage displayed in three places: live output after each researcher finishes, per-dimension detail in SUMMARY.md, per-phase aggregates in /gsd:progress." This step implements the second place (per-dimension detail in SUMMARY.md).

    Add a `## Research Token Usage` section to the summary template, positioned between `## Performance` and `## Accomplishments`. The section is conditional -- only include it when a RESEARCH.md with a `<token_report>` exists for the current phase.

    Add this to the template's markdown block:
    ...
    *Estimates based on self-reported token usage. Actual API billing may differ.*
    ...
    Also add guidance in the `<frontmatter_guidance>` or `<guidelines>` section explaining how the executor should populate this section:

    1. After completing all tasks, check if a RESEARCH.md exists for this phase: `ls ${PHASE_DIR}/*-RESEARCH.md 2>/dev/null`
    2. If it exists, parse the `<token_report>` block from the end of the file
    3. Extract: dimensions, model, input_tokens, output_tokens, estimated_cost_usd
    4. Populate the Research Token Usage section with these values
    5. If no RESEARCH.md or no `<token_report>` exists, omit the section entirely (do not include an empty section)

    The tone should match the "informed, not anxious" principle -- present data without judgment.
  </action>
  <verify>
    ...
    Read ~/.claude/get-shit-done/templates/summary.md and verify "## Research Token Usage" section exists in the template with conditional inclusion guidance and token_report parsing instructions.
  </verify>
  <done>config.json contains token_prices with current Anthropic model pricing. STATE.md template contains a ## Cost Tracker section with table format and auto-pruning documentation. Summary template contains a conditional ## Research Token Usage section with per-dimension cost detail populated from RESEARCH.md token_report data.</done>
</task>
...
3. Summary template has conditional ## Research Token Usage section with per-dimension detail from token_report
4. gsd-tools.cjs state update-cost works: accepts args, writes to STATE.md, maintains running total
5. Cost Tracker table respects 10-row cap
6. progress.md shows cost summary from STATE.md data
7. progress.md omits cost summary when no data exists
8. All cost displays use "estimated" language
</verification>

<success_criteria>
- Token usage tracked per phase execution via gsd-tools.cjs (OBS-02)
- Cost summary displayed in progress reports (OBS-03)
- Per-dimension token detail rendered in SUMMARY.md via template (per locked decision: "per-dimension detail in SUMMARY.md")
